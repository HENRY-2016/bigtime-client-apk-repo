...
// Bigtime
function LoadBigtimeUsingGitHub ()
{
    if(!navigator.onLine)
    {
    ...
    if(navigator.onLine)
    {window.location='https://henry-2016.github.io/bigtime-client-server-repo/'}
    
}

function LoadBigtimeUsingRaspberry ()

{
    if(!navigator.onLine)
        {CreateCustomAlertBox ()}
    if(navigator.onLine)
        {window.location="http://192.168.43.140:80/bigtime-client-server-repo/index.html"}
}
